...
class CardDAO(context: Context) {
    ...
    suspend fun updateIsLearnedCardById(id: String, isLearned: Int): Long {
        ...
        return try {
            ...
            val documentSnapshot = db.collection("cards").document(id).get().await()

            if (documentSnapshot.exists()) {
                // If the document exists, create a map with the updated "is_learned" value
                ...
                Log.e("CardDAO", "Document with ID: $id does not exist.")
                0L // Return 0 to indicate failure
            }
        } catch (e: Exception) {
    ...
    fun checkCardExist(card_id: String): Boolean {
        val cardRef = db.collection("cards").document(card_id)

        val task = cardRef.get()
        val result: Boolean

        try {
            // Wait for the task to complete
            Tasks.await(task)
            result = task.isSuccessful && task.result?.exists() == true
        } catch (e: Exception) {
            Log.e("CardDAO", "checkCardExist: ", e)
            return false // Assume card does not exist on failure
        }
    ...
    suspend fun getCardByIsLearned(flashcardId: String, isLearned: Int): ArrayList<Card> {
        ...
        try {
            // Firestore query to get all cards with the matching flashcard ID and is_learned status
            val querySnapshot = db.collection("cards")
                .whereEqualTo("flashcard_id", flashcardId)
            ...
            // Iterate through the query results and map them to Card objects
            for (document in querySnapshot.documents) {
                val card = Card().apply {
            ...
            Log.e("CardDAO", "getCardByIsLearned: ${e.message}", e)
        }
    ...
    suspend fun getAllCardByFlashCardId(flashcardId: String): ArrayList<Card> {
        ...
        try {
            // Firestore query to get all cards with the matching flashcard ID
            val querySnapshot = db.collection("cards")
                .whereEqualTo("flashcard_id", flashcardId)
                .get()
                .await()
            ...
            Log.e("CardDAO", "getAllCardByFlashCardId: ${e.message}", e)
        }
